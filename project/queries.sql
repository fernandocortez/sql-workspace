...
SELECT order_id, sum(labor_hour*labor_cost_per_hour+part_cost) AS total_cost, year, make, model, mileage FROM tbl_order_service NATURAL JOIN tbl_service NATURAL JOIN tbl_rate NATURAL JOIN tbl_vehicle_order NATURAL JOIN tbl_vehicle GROUP BY order_id HAVING total_cost > 550 ORDER BY order_id;
...
SELECT customer_id, first_name, last_name, sum(labor_hour*labor_cost_per_hour+part_cost) AS total_cost FROM tbl_customer NATURAL JOIN tbl_customer_vehicle NATURAL JOIN tbl_vehicle_order NATURAL JOIN tbl_order_service NATURAL JOIN tbl_service NATURAL JOIN tbl_rate GROUP BY customer_id ORDER BY last_name, first_name;
...
SELECT service_id, name, count(service_id) AS service_count, sum(labor_hour*labor_cost_per_hour+part_cost) AS service_income FROM tbl_order_service NATURAL JOIN tbl_service NATURAL JOIN tbl_rate GROUP BY service_id ORDER BY service_id;
...
SELECT first_name, last_name, customer_id, sum(labor_hour*labor_cost_per_hour+part_cost) AS total_cost FROM tbl_customer NATURAL JOIN tbl_customer_vehicle NATURAL JOIN tbl_vehicle_order NATURAL JOIN tbl_order_service NATURAL JOIN tbl_service NATURAL JOIN tbl_rate GROUP BY customer_id;
...
SELECT vehicle_id, year, make, model, mileage, count(order_id) AS order_count, sum(labor_hour*labor_cost_per_hour+part_cost) AS vehicle_cost FROM tbl_vehicle_order NATURAL JOIN tbl_vehicle NATURAL JOIN tbl_order_service NATURAL JOIN tbl_service NATURAL JOIN tbl_rate GROUP BY vehicle_id;

-- What is the total income of this auto shop?
SELECT total(labor_hour*labor_cost_per_hour+part_cost) AS total_income FROM tbl_order_service NATURAL JOIN tbl_service NATURAL JOIN tbl_rate;
